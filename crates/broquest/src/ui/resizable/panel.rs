//! Vendored from gpui-component (rev 1505b148) `crates/ui/src/resizable/`.
//!
//! Forked so the resize handle can be made invisible: broquest draws its panels
//! as bordered rounded cards, and the upstream handle paints a 1px line down the
//! full length of the split, which extends past those cards' rounded corners.
//! See `ResizablePanelGroup::invisible_handles`.
...
impl ResizablePanelGroup {
    ...
    /// Keep the resize handles draggable but stop them painting their 1px line.
    ///
    /// Use this when the panels themselves are bordered cards: the handle's line
    /// spans the full length of the split and would otherwise extend past the
