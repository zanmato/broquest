...
//!
//! The only behavioural change is `ResizablePanelGroup::invisible_handles`; see
//! `panel.rs` and `resize_handle.rs`. Keeping the rest verbatim makes it cheap
//! to re-sync with upstream.

// Kept verbatim from upstream, so don't lint it as if it were our own code:
// broquest only uses part of the API, and the rest should stay in place for the
// next re-sync.
#![allow(
    dead_code,
